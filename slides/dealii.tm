...
<\body>
  <\hide-preamble>
    <assign|pd|<\macro|num|den>
      <frac|\<partial\> <arg|num>|\<partial\> <arg|den>>
    </macro>>

    <assign|od|<\macro|num|den>
      <frac|\<mathd\><arg|num>|\<mathd\><arg|den>>
    </macro>>

    <assign|ipa|<macro|body|\<langle\><arg|body>\<rangle\>>>

    <assign|Cmn|<macro|\<bbb-C\><rsup|m\<times\>n>>>
    <assign|Cm|<macro|\<bbb-C\><rsup|m>>>
    <assign|Cn|<macro|\<bbb-C\><rsup|n>>>
    <assign|Cmm|<macro|\<bbb-C\><rsup|m\<times\>m>>>
    <assign|Cnn|<macro|\<bbb-C\><rsup|n\<times\>n>>>

    <assign|Rmn|<macro|\<bbb-R\><rsup|m\<times\>n>>>
    <assign|Rm|<macro|\<bbb-R\><rsup|m>>>
    <assign|Rn|<macro|\<bbb-R\><rsup|n>>>
    <assign|Rmm|<macro|\<bbb-R\><rsup|m\<times\>m>>>
    <assign|Rnn|<macro|\<bbb-R\><rsup|n\<times\>n>>>

    <assign|hatU|<macro|<wide|U|^>>>

    <assign|hatSi|<macro|<wide|\<Sigma\>|^>>>
  ...
  <\slideshow>
    <\slide>
      ...
      <small|<\with|font-shape|italic>
        Praveen Chandrashekar

        Centre for Applicable Mathematics, Tata Institute of Fundamental
        Research, Bangalore \U 560065, India.

        email: <tt|praveen@math.tifrbng.res.in><space|3em>www:
        <verbatim|http://cpraveen.github.io>
    ...
    <\slide>
      ...
      <\equation*>
        u<rsub|h><around*|(|x|)>=<big|sum><rsub|i=0><rsup|N-1>u<rsub|i>
        \<phi\><rsub|i><around*|(|x|)>
      </equation*>

      The basis functions <math|\<phi\><rsub|i>> are piecewise polynomials
      with compact support. The sparsity pattern can be thought of as a
      <math|N\<times\>N> matrix <math|S>

      <\equation*>
        S<rsub|i j>=<choice|<tformat|<table|<row|<cell|1,>|<cell|supp<around*|(|\<phi\><rsub|i>|)>\<cap\>supp<around*|(|\<phi\><rsub|j>|)>
        is not empty>>|<row|<cell|0,>|<cell|otherwise>>>>>
      </equation*>

      Examine the sparsity pattern for a DG space.
    ...
    <\slide>
      ...
      <\equation*>
        u<rsub|h><around*|(|x|)>=<big|sum><rsub|i=0><rsup|N-1>u<rsub|i><rsup|K>
        \<phi\><rsub|i><rsup|K><around*|(|x|)>,<space|2em>x\<in\>K
      </equation*>

      Here <math|<around*|{|\<phi\><rsub|i><rsup|K>|}>> are the shape
      functions with support on element <math|K>. The corresponding dofs
      ...
      <\padded-center>
        <math|u<rsub|i><rsup|K>> = <verbatim|solution(global_dof_indices[i])>
      </padded-center>

      The gradient of the solution is similarly given by

      <\equation*>
        \<nabla\>u<rsub|h><around*|(|x|)>=<big|sum><rsub|i=0><rsup|N-1>u<rsub|i><rsup|K>
        \<nabla\>\<phi\><rsub|i><rsup|K><around*|(|x|)>
      </equation*>
    </slide>
    ...
    <\slide>
      ...
      <\equation*>
        -<big|int><rsub|K>f<around*|(|u<rsub|h>|)>\<cdot\>\<nabla\>\<phi\><rsub|i>
        \<mathd\>x
      </equation*>

      Internal face integral

      <\equation*>
        +<big|int><rsub|F>H<around*|(|u<rsub|h><rsup|->,u<rsub|h><rsup|+>,n<rsub|->|)>
        \<phi\><rsub|i><rsup|-> \<mathd\>s,<space|2em>-<big|int><rsub|F>H<around*|(|u<rsub|h><rsup|->,u<rsub|h><rsup|+>,n<rsub|->|)>
        \<phi\><rsub|j><rsup|+> \<mathd\>s
      </equation*>

      Boundary face integral

      <\equation*>
        <big|int><rsub|F>H<around*|(|u<rsub|h>,u<rsub|b>,n|)> \<phi\><rsub|i>
        \<mathd\>s
      </equation*>
    </slide>
...
<\initial>
  <\collection>
    ...
    <associate|par-par-sep|0.2fn>
    <associate|par-sep|0.15fn>
